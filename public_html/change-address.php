...
<style>
    ...
    .change-address-page .field.big {
        flex: 6;
    }

    .change-address-page .field.small {
        flex: 4;
    }

    .change-address-page .field.huge {
        flex: 10;
    }
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            form.addEventListener('submit', (e) => {
                ...
                fetch('<?= BASE_URL ?>actions/validate-zip.php', {
                        method: 'POST',
                        body: formData
                    })
                    .then(res => res.json())
                    .then(validation => {
                        if (!validation.success) {
                            showAlertModal(
                                validation.message || 'The ZIP code does not match the selected state.',
                                () => {}
                            );
                            return;
                        }
                        return fetch('<?= BASE_URL ?>actions/change-address.php', {
                            method: 'POST',
                            body: formData
                        });
                    })
